...
class App(tk.Tk):
    ...
    def __init__(self):
        ...
        # Initialize record management system
        self.rms = RMS()
    ...
    def build_clients_tab(self):
        """build the Clients tab interface"""
        f = self.tab_clients
        ...
        # Row 0: Client ID
        ttk.Label(form, text="Client ID").grid(row=0, column=0, sticky="w",
                                               padx=1, pady=4)
        self.ent_client_id = ttk.Entry(form, width=22, state="readonly",
                                       style="Gray.TEntry")
        self.ent_client_id.grid(row=0, column=1, sticky="w", padx=1, pady=4)

        #Row 1: Record Type
        ttk.Label(form, text="Record Type").grid(row=1, column=0, sticky="w", 
                                                 padx=1, pady=4)
        self.ent_client_type = ttk.Entry(form, width=22, state="readonly", 
                                         style="Gray.TEntry")
        ...
        # Row 3: Country
        ttk.Label(form, text="Country*").grid(row=3, column=0, sticky="w", 
                                              padx=1, pady=4)
        self.cmb_country = ttk.Combobox(form, width=21,
                                        values=self.rms.list_countries(), state="readonly")
        self.cmb_country.grid(row=3, column=1, sticky="w", padx=1, pady=4)
        self.cmb_country.bind("<<ComboboxSelected>>", 
                              self.on_client_country_selected)

        # Row 4: Address Line 1
        ttk.Label(form, text="Address Line 1*").grid(row=4, column=0, sticky="w", 
                                                     padx=1, pady=4)
        self.ent_addr1 = ttk.Entry(form, width=22)
        self.ent_addr1.grid(row=4, column=1, padx=1, sticky="w", pady=4)

        # Row 5: Address Line 2
        ttk.Label(form, text="Address Line 2").grid(row=5, column=0, sticky="w", 
                                                    padx=1, pady=4)
        self.ent_addr2 = ttk.Entry(form, width=22)
        self.ent_addr2.grid(row=5, column=1, sticky="w", padx=1, pady=4)

        # Row 6: Address Line 3
        ttk.Label(form, text="Address Line 3").grid(row=6, column=0, sticky="w", 
                                                    padx=1, pady=4)
        self.ent_addr3 = ttk.Entry(form, width=22)
        self.ent_addr3.grid(row=6, column=1, sticky="w", padx=1, pady=4)

        # Row 7: City
        ttk.Label(form, text="City*").grid(row=7, column=0, sticky="w", 
                                           padx=1, pady=4)
        self.cmb_city = ttk.Combobox(form, width=21, 
                                     values=self.rms.list_cities(), 
                                     state="readonly")
        self.cmb_city.grid(row=7, column=1, sticky="w", padx=1, pady=4)

        # Row 8: State/County
        ttk.Label(form, text="State/County*").grid(row=8, column=0, sticky="w", 
                                                   padx=1, pady=4)
        self.ent_state = ttk.Entry(form, width=22)
        self.ent_state.grid(row=8, column=1, sticky="w", 
                            padx=1, pady=4)

        # Row 9: ZIP Code/Postcode
        ttk.Label(form, text="ZIP Code/Postcode*").grid(row=9, column=0, 
                                                        sticky="w", 
                                                        padx=1, pady=4)
        ...
        # Row 10: Phone Number
        ttk.Label(form, text="Phone Number*").grid(row=10, column=0, 
                                                   sticky="w", 
                                                   padx=1, pady=4)
        ...
        ttk.Button(form, text="Create New Record", 
                   command=self.on_client_create).grid(row=11, 
                                                       column=0, sticky="sw", 
                                                       padx=4, pady=4)
        ttk.Button(form, text="Update Record", 
                   command=self.on_client_update).grid(row=11, 
                                                       column=1, sticky="se", 
                                                       padx=4, pady=4)
        ttk.Button(form, text="Delete Record", 
                   command=self.on_client_delete).grid(row=12, 
                                                       column=0, sticky="sw", 
                                                       padx=4, pady=4)
        ttk.Button(form, text="Save Changes", 
                   command=self.on_save_all).grid(row=12, 
                                                  column=1, sticky="se", 
                                                  padx=4, pady=4) # Does nothing yet
        ...
        # Define search bar
        ttk.Label(bar, text="Search (by Client ID/Phone Number/Name)").pack(
            side="left"
            )
        ...
        # Define buttons
        ttk.Button(bar, text="Find", 
                   command=self.on_client_search).pack(side="left", padx=4)
        ttk.Button(bar, text="Show All", 
                   command=self.refresh_clients).pack(side="left", padx=4)

        # Define list
        self.tree_clients = ttk.Treeview(
            right, columns=("Client ID", "Name", "Phone Number"), 
            show="headings", height=10)
        
        # for each column, define heading and column properties
        for c, w in (("Client ID", 100), ("Name", 260), 
                     ("Phone Number", 160)):
    ...
    def on_client_country_selected(self, _evt=None):
        """Update city list when a country is selected"""
    ...
    def _collect_client_form(self) -> dict:
        """Return client from data as a dictionary"""
        return dict(
    ...
    def refresh_clients(self, rows=None):
        """Refresh the clients list; if rows is given, use it instead of all clients"""
        if rows is None:
        ...
        for r in rows:
            self.tree_clients.insert(
                "", "end", 
                values=(r.get("client_id"), r.get("Name", ""), 
                        r.get("Phone", ""))
            )

    def on_client_select(self, _evt):
        """When a client is selected from the list, populate the form with its data"""
        ...
        self.ent_name.insert(0, r.get("Name", ""))
        
        self.ent_addr1.delete(0, "end")
        self.ent_addr1.insert(0, r.get("Address1", ""))
        
        self.ent_addr2.delete(0, "end")
        self.ent_addr2.insert(0, r.get("Address2", ""))
        
        self.ent_addr3.delete(0, "end")
        self.ent_addr3.insert(0, r.get("Address3", ""))
        
        self.cmb_city.set(r.get("City", ""))
        self.ent_state.delete(0, "end")
        self.ent_state.insert(0, r.get("State", ""))
        self.ent_zip.delete(0, "end")
        self.ent_zip.insert(0, r.get("Zip", ""))
        self.cmb_country.set(r.get("Country", ""))
        self.ent_phone.delete(0, "end")
        self.ent_phone.insert(0, r.get("Phone", ""))

    def on_client_create(self):
        """Create a new client record from the form data"""
    ...
    def on_client_update(self):
        """Update the selected client record with the form data"""
    ...
    def on_client_search(self):
        """Search clients by ID, Name, or Phone Number"""
    ...
    def build_airlines_tab(self):
        """build the Airlines tab interface"""
        f = self.tab_airlines
        ...
        # Row 0: Airline ID
        ttk.Label(form, text="Airline ID").grid(row=0, 
                                                column=0, sticky="w", 
                                                padx=1, pady=4)
        self.ent_airline_id = ttk.Entry(form, width=22, 
                                        state="readonly", style="Gray.TEntry")
        self.ent_airline_id.grid(row=0, column=1, sticky="w", 
                                 padx=1, pady=4)

        # Row 1: Record Type
        ttk.Label(form, text="Record Type").grid(row=1, column=0, 
                                                 sticky="w", 
                                                 padx=1, pady=4)
        self.ent_airline_type = ttk.Entry(form, width=22, 
                                          state="readonly", 
                                          style="Gray.TEntry")
        self.ent_airline_type.grid(row=1, column=1, sticky="w", 
                                   padx=1, pady=4)
        var = tk.StringVar(value="airline")
        self.ent_airline_type.configure(textvariable=var)

        # Row 2: Company Name
        ttk.Label(form, text="Company Name").grid(row=2, column=0, 
                                                  sticky="w", 
                                                  padx=1, pady=4)
        self.ent_company = ttk.Entry(form, width=22)
        self.ent_company.grid(row=2, column=1, sticky="w", 
                              padx=1, pady=4)

        # Action buttons Rows 3-4
        #bar = ttk.Frame(form); bar.grid(row=2, column=0, columnspan=4, sticky="w", padx=4, pady=8)
        ttk.Button(form, text="Create New Record", 
                   command=self.on_airline_create).grid(row=11, column=0, 
                                                        sticky="sw", 
                                                        padx=4, pady=4)
        ttk.Button(form, text="Update Record", 
                   command=self.on_airline_update).grid(row=11, column=1, 
                                                        sticky="se", 
                                                        padx=4, pady=4)
        ttk.Button(form, text="Delete Record", 
                   command=self.on_airline_delete).grid(row=12, column=0, 
                                                        sticky="sw", padx=4, 
                                                        pady=4)
        ttk.Button(form, text="Save Changes", 
                   command=self.on_save_all).grid(row=12, column=1, 
                                                  sticky="se", padx=4, 
                                                  pady=4) # Does nothing yet

        # Right Panel search bar + list
        bar = ttk.Frame(right)
        bar.pack(side="top", fill="x")
        ...
        self.tree_airlines = ttk.Treeview(
            right, 
            columns=("Airline ID", "Company Name"), 
            show="headings", 
            height=10
            )
        for c, w in (("Airline ID", 180), 
                     ("Company Name", 360)):
            self.tree_airlines.heading(c, text=c)
            self.tree_airlines.column(c, width=w, anchor="w")
        
        self.tree_airlines.pack(fill="both", expand=True, 
                                padx=0, pady=8)
        self.tree_airlines.bind("<<TreeviewSelect>>", 
                                self.on_airline_select)

    def refresh_airlines(self):
        """Refresh the airlines list"""
        self.tree_airlines.delete(*self.tree_airlines.get_children())
        for r in self.rms.airlines:
            self.tree_airlines.insert(
                "", "end", 
                values=(r.get("airline_id"), 
                        r.get("CompanyName", ""))
            )

    def on_airline_select(self, _evt):
        """When an airline is selected from the list, populate the form with its data"""
    ...
    def on_airline_create(self):
        """Create a new airline record from the form data"""
        if not messagebox.askyesno("Confirm", "Create this airline?"):
            return
        try:
            r = self.rms.create_airline(
                {"CompanyName": self.ent_company.get().strip()}
                )
            messagebox.showinfo("OK", 
                                f"Created airline {r['airline_id']}")
            self.refresh_airlines()
            self.refresh_flights()
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def on_airline_update(self):
        """Update the selected airline record with the form data"""
        ...
        if not messagebox.askyesno("Confirm", f"Update airline {aid}?"):
            return
        try:
            r = self.rms.update_airline(
                int(aid), 
                {"CompanyName": self.ent_company.get().strip()}
            )
            messagebox.showinfo("OK", 
                                f"Updated airline {r['airline_id']}")
            self.refresh_airlines()
            self.refresh_flights()
        except Exception as e:
            messagebox.showerror("Error", str(e))
    ...
    def on_airline_delete(self):
        ...
        if not messagebox.askyesno(
            "Confirm", f"Delete airline {aid} and its flights?"
            ):
            return
        try:
    ...
    def build_flights_tab(self):
        """build the Flights tab interface"""
        f = self.tab_flights
        ...
        ttk.Label(form, text="Flight ID").grid(row=0, column=0, 
                                               sticky="w", 
                                               padx=1, pady=4)
        self.ent_fid = ttk.Entry(form, width=22, state="readonly", 
                                 style="Gray.TEntry")
        self.ent_fid.grid(row=0, column=1, sticky="w", 
                          padx=1, pady=4)

        # Row 1: Client dropdown
        ttk.Label(form, text="Client").grid(row=1, column=0, 
                                            sticky="w", 
                                            padx=1, pady=4)
        self.cmb_client = ttk.Combobox(
            form, width=21, values=self.rms.list_clients_combo(), 
            state="readonly"
        )
        self.cmb_client.grid(row=1, column=1, sticky="w", padx=1, pady=4)

        # Row 2: Airline dropdown
        ttk.Label(form, text="Airline").grid(row=2, column=0, 
                                             sticky="w", 
                                             padx=1, pady=4)
        self.cmb_airline = ttk.Combobox(
            form, width=21, values=self.rms.list_airlines_combo(), 
            state="readonly"
        )
        self.cmb_airline.grid(row=2, column=1, sticky="w", 
                              padx=1, pady=4)
        ...
        bar = ttk.Frame(form)
        bar.grid(row=3, column=0, columnspan=3, sticky="w", pady=4)
        ttk.Label(bar, text="Date (Y/M/D H:M)").grid(row=3,column=0, 
                                                     sticky="w", 
                                                     pady=4)
        ...
        self.cbMin = ttk.Combobox(bar, width=2, values=minutes, 
                                  state="readonly")
        for i, cb in enumerate(
            [self.cbY, self.cbM, self.cbD, self.cbH, self.cbMin], 
            start=1):
            cb.grid(row=3, column=i, sticky="w", pady=4)

        # Rows 4-5: Start and End Cities
        ttk.Label(form, text="Start City").grid(row=4, column=0, 
                                                sticky="w", padx=1, 
                                                pady=4)
        self.cmb_start = ttk.Combobox(
            form, width=21, values=self.rms.list_cities(), 
            state="readonly"
        )
        self.cmb_start.grid(row=4, column=1, sticky="w", padx=1, pady=4)
        
        ttk.Label(form, text="End City").grid(row=5, column=0, 
                                              sticky="w", 
                                              padx=1, pady=4)
        self.cmb_end = ttk.Combobox(
            form, width=21, values=self.rms.list_cities(), 
            state="readonly"
        )
        ...
        ttk.Button(form, text="Create", 
                   command=self.on_flight_create).grid(row=6, column=0, 
                                                       sticky="sw", 
                                                       padx=4, pady=4)
        ttk.Button(form, text="Update", 
                   command=self.on_flight_update).grid(row=6, column=1, 
                                                       sticky="se", 
                                                       padx=4, pady=4)
        ttk.Button(form, text="Delete", 
                   command=self.on_flight_delete).grid(row=7, column=0, 
                                                       sticky="sw", 
                                                       padx=4, pady=4)
        ttk.Button(form, text="Save Changes", 
                   command=self.on_save_all).grid(row=7, column=1, 
                                                  sticky="se", 
                                                  padx=4, pady=4) # Does nothing yet

        # Right Panel search + foreign key combo search
        sbar = ttk.Frame(right)
        sbar.pack(side="top", fill="x")

        ttk.Label(sbar, 
                  text="Search by Client ID/Client Name/Phone Number").grid(
                      row=0, column=0, sticky="w"
                      )
        self.ent_fsearch = ttk.Entry(sbar, width=32)
        self.ent_fsearch.grid(row=0, column=1, sticky="w")
        ttk.Button(sbar, text="Find", 
                   command=self.on_flight_search).grid(row=0, column=2, 
                                                       padx=4)
        ttk.Button(sbar, text="Filter", 
                   command=self.on_flight_search_fk).grid(row=1, column=2, 
                                                          padx=4)
        ttk.Button(sbar, text="Show All", 
                   command=self.refresh_flights).grid(row=0, column=3, 
                                                      padx=4)

        ttk.Label(sbar, text="Filter by Client & Airline").grid(
            row=1, column=0, sticky="w"
        )
        self.cmb_fk_client = ttk.Combobox(
            sbar, width=31, values=self.rms.list_clients_combo(), 
            state="readonly"
        )
        self.cmb_fk_client.grid(row=1, column=1, sticky="w")
        self.cmb_fk_airline = ttk.Combobox(
            sbar, width=31, values=self.rms.list_airlines_combo(), 
            state="readonly"
        )
        self.cmb_fk_airline.grid(row=2, column=1, sticky="w")

        # List of flights
        cols = ("ID", "Client Name", "Phone Number", "Airline Name", 
    ...
    def _pick_id_from_combo(txt: str) -> int:
        """Extract the ID from a combo box text"""
        try:
            if txt and "-" in txt:
    ...
    def _collect_flight_form(self) -> dict:
        """Collect flight form data and return as a dictionary"""
    ...
    def refresh_flights(self, rows=None):
        """Refresh the flights list; if rows is given, use it instead of all flights"""
        if rows is None:
            
            # Build rows with joined data
            rows = []
            for f in self.rms.flights:
                c = self.rms._find(self.rms.clients, "client_id", 
                                   int(f.get("client_id", 0))) or {}
                a = self.rms._find(self.rms.airlines, "airline_id", 
                                   int(f.get("airline_id", 0))) or {}
                rows.append({
                    "ID": f.get("ID"),
        ...
        for r in rows:
            self.tree_flights.insert(
                "", "end", 
                values=(r["ID"], r["ClientName"], r["Phone"], r["Airline"], 
                        r["Date"], r["StartCity"], r["EndCity"])
            )
    ...
    def on_flight_select(self, _evt):
        ...
        # Restore form
        c = self.rms._find(
            self.rms.clients, "client_id", int(row.get("client_id", 0))
            ) or {}
        a = self.rms._find(
            self.rms.airlines, "airline_id", int(row.get("airline_id", 0))
            ) or {}
    ...
    def on_flight_create(self):
        """Create a new flight record from the form data"""
    ...
    def on_flight_update(self):
        """Update the selected flight record with the form data"""
