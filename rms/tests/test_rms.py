...
class TestRMSUtils(unittest.TestCase):
    ...
    def test_next_id_with_data(self):
        ...
        print(
            f"Test Details: Rows with max ID 5 → _next_id returned {result} (expected 6)"
        )
    ...
    def test_next_id_ignore_invalid(self):
        ...
        result = self.rms._next_id(rows, "client_id")
        self.assertEqual(
            result, 3, "Should ignore non-numeric IDs and return (valid max ID + 1)"
        )
        print(
            f"Test Details: Rows with invalid ID 'abc' → _next_id returned {result} (expected 3)"
        )
    ...
    def test_find_exist(self):
        ...
        print(
            f"Test Details: Found record for ID 2 → {result} (matches expected record)"
        )
    ...
    def test_find_not_exist(self):
        ...
        print(
            f"Test Details: Search for non-existent ID 999 → _find returned {result} (expected None)"
        )
    ...
    def test_index_of_exist(self):
        ...
        self.assertEqual(
            index, 1, "Should return the correct index of the target record"
        )
        self.assertEqual(
            record, rows[1], "Should return the record matching the target ID"
        )
        print(
            f"Test Details: Found ID 8 at index {index} → Record: {record} (expected)"
        )

    # Test: _index_of returns (-1, None) when the target ID does not exist
    def test_index_of_not_exist(self):
        rows = [{"airline_id": 5}]
        index, record = self.rms._index_of(
            rows, "airline_id", 10
        ...
        print(
            f"Test Details: Search for non-existent ID 10 → Index: {index}, Record: {record} (expected (-1, None))"
        )
...
class TestRMSClient(unittest.TestCase):
    ...
    def test_create_client_valid(self):
        ...
        self.assertEqual(
            result["Type"], "client", "Client record 'Type' should be 'client'"
        )
        self.assertEqual(
            result["client_id"], 1, "First client created should have ID=1"
        )
        self.assertEqual(result["Name"], "Bob", "Client name should match input")
        self.assertEqual(
            len(self.rms.clients), 1, "Client list should have 1 new record"
        )

        print(
            f"Test Details: Created client with ID {result['client_id']} (Name: {result['Name']})"
        )
    ...
    def test_create_client_missing_fields(self):
        ...
        print(
            f"Test Details: Missing required fields → Error raised: '{error_msg}' (expected)"
        )
    ...
    def test_update_client_not_exist(self):
        ...
        self.assertIn(
            ...
            error_msg,
            "Error should say client ID 999 is not found",
        )

        print(
            f"Test Details: Update non-existent client 999 → Error raised: '{error_msg}' (expected)"
        )
    ...
    def test_delete_client(self):
        ...
        self.assertEqual(
            len(self.rms.flights), 0, "Associated flights should be deleted too"
        )

        print(
            f"Test Details: Deleted client ID 1 → Client list size: {len(self.rms.clients)} (0), Flight list size: {len(self.rms.flights)} (0)"
        )
    ...
    def test_search_clients(self):
        ...
        search_by_name = self.rms.search_clients(
            "ali"
        )  # Search by name substring ("ali" in "Alice")
        search_by_phone = self.rms.search_clients(
            "138"
        )  # Search by phone substring ("138" in "13800138000")
        ...
        self.assertEqual(
            len(search_by_name), 1, "Search by name 'ali' should find 1 client"
        )
        self.assertEqual(
            len(search_by_phone), 1, "Search by phone '138' should find 1 client"
        )
        self.assertEqual(
            len(search_no_match), 0, "Search for 'bob' should return 0 results"
        )

        print(
            f"Test Details: Search results → ID '1': {len(search_by_id)}, Name 'ali': {len(search_by_name)}, Phone '138': {len(search_by_phone)}, 'bob': {len(search_no_match)}"
        )
...
class TestRMSAirline(unittest.TestCase):
    ...
    def test_create_airline_valid(self):
        ...
        # Validate return value and in-memory state
        self.assertEqual(
            result["Type"], "airline", "Airline record 'Type' should be 'airline'"
        )
        self.assertEqual(
            result["airline_id"], 1, "First airline created should have ID=1"
        )
        self.assertEqual(
            len(self.rms.airlines), 1, "Airline list should have 1 new record"
        )

        print(
            f"Test Details: Created airline with ID {result['airline_id']} (Name: {result['CompanyName']})"
        )
    ...
    def test_create_airline_missing_name(self):
        ...
        self.assertIn(
            ...
            error_msg,
            "Error should say CompanyName is required",
        )

        print(
            f"Test Details: Empty CompanyName → Error raised: '{error_msg}' (expected)"
        )
    ...
    def test_delete_airline(self):
        ...
        print(
            f"Test Details: Deleted airline ID 1 → Airline list size: {len(self.rms.airlines)} (0), Flight list size: {len(self.rms.flights)} (0)"
        )
...
class TestRMSFlight(unittest.TestCase):
    ...
    def test_create_flight_valid(self):
        ...
        # Validate return value and in-memory state
        self.assertEqual(
            result["Type"], "flight", "Flight record 'Type' must be 'flight'"
        )
        self.assertEqual(result["ID"], 1, "First valid flight should have ID=1")
        self.assertEqual(
            result["StartCity"], "Hong Kong", "Flight start city should match input"
        )
        self.assertEqual(
            len(self.rms.flights), 1, "Flight list should add 1 new record"
        )

        # Log details to console
        print(
            f"Test Details: Created flight ID {result['ID']} → {result['StartCity']} → {result['EndCity']}"
        )
    ...
    def test_create_flight_invalid_date(self):
        ...
        invalid_date_data = {
            "client_id": 1,
            "airline_id": 1,
            "Date": "2024/12/31 23:55",  # Invalid: Should be "YYYY-MM-DD HH:MM"
            "StartCity": "Hong Kong",
            "EndCity": "London",
        }
        ...
        # Log details to console
        print(
            f"Test Details: Invalid date format → Error raised: '{error_msg}' (expected)"
        )
    ...
    def test_create_flight_missing_client(self):
        ...
        error_msg = str(exc_context.exception)
        self.assertIn(
            "client_id 999 not found",
            error_msg,  # Match the actual error message
            "Error should say linked client does not exist",
        )

        # Log details to console
        print(
            f"Test Details: Non-existent client ID 999 → Error raised: '{error_msg}' (expected)"
        )
    ...
    def test_create_flight_missing_airline(self):
        ...
        # Log details to console
        print(
            f"Test Details: Non-existent airline ID 999 → Error raised: '{error_msg}' (expected)"
        )
    ...
    def test_search_flights(self):
        ...
        search_by_client_name = self.rms.search_flights(
            "charlie"
        )  # Matches client name substring
        search_by_client_id = self.rms.search_flights("1")  # Matches client ID
        search_by_client_phone = self.rms.search_flights(
            "139"
        )  # Matches client phone substring
        search_no_client_match = self.rms.search_flights(
            "bob"
        )  # No matching client (no results)

        # Step 3: Test that airline-based searches return 0 (since current logic doesn't support them)
        search_by_airline = self.rms.search_flights(
            "air china"
        )  # No client match → 0 results

        # Step 4: Validate results (align with current search_flights behavior)
        self.assertEqual(
            len(search_by_client_name),
            1,
            "Search by client name 'charlie' should find 1 flight",
        )
        self.assertEqual(
            len(search_by_client_id), 1, "Search by client ID '1' should find 1 flight"
        )
        self.assertEqual(
            len(search_by_client_phone),
            1,
            "Search by client phone '139' should find 1 flight",
        )
        self.assertEqual(
            len(search_no_client_match),
            0,
            "Search for 'bob' (no client match) should return 0 results",
        )
        self.assertEqual(
            len(search_by_airline),
            0,
    ...
    def test_delete_flight(self):
        ...
        self.rms.delete_flight(
            flight_id=1
        )  # Assumes RMS has a delete_flight method (matches CRUD pattern)
        ...
        self.assertEqual(
            len(self.rms.clients), 1, "Client should remain (not deleted with flight)"
        )
        self.assertEqual(
            len(self.rms.airlines), 1, "Airline should remain (not deleted with flight)"
        )

        # Log details to console
        print(
            f"Test Details: Deleted flight ID 1 → "
